...
This skill manages the transition from raw request chaos to a structured MVR by first locating the vault root (`vault_root_name` from devops config).

## Workflow

### 1. Vault Discovery & Capabilities
1.  **Locate Vault**: Search for the `vault_root_name` directory (from devops config) to ensure absolute pathing.
2.  **Define Routing Paths**:
    - **Approved**: `[VAULT_ROOT]/Workflows/request-triage/intake-sheriff-approved/`
    - **Unapproved**: `[VAULT_ROOT]/Workflows/request-triage/intake-sheriff-unapproved/`
...
3.  **CONFLUENCE_REFERENCES Handling** (before general routing):
    - **IF** a file named `CONFLUENCE_REFERENCES` exists in the input:
      1.  Read the file. Each non-empty, non-comment line is a Confluence page URL or page ID.
          ...
          https://{jira_domain}/wiki/spaces/DEVOPS/pages/123456789/Page+Title
          987654321
          ```
