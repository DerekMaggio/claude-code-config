...
2. Mapping Salesforce Customer Names to Jira Customer custom field options.
...
4. **If logged in**:
   - Run `sf alias set {salesforce_org_alias}=<username>` (use `salesforce_org_alias` from devops config) to ensure the alias is active.
...
1. **Fetch Field Data:**
   ...
   - projectIdOrKey: jira_project_key (from devops config)
   - issueTypeId: jira_customer_deployment_type_id (from devops config)
   ```
...
3. **Build Customer Lookup Map:**
   - Locate `jira_customer_field` (from devops config).
...
3. **Perform Customer Name Match:**
   ...
   - **If No Match:** Store `jira_option_id = jira_default_customer_id` (from devops config) and label status as "MATCH_FAIL".
