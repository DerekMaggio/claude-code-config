...
install_node() {
    ...
    nvm install --lts > /dev/null 2>&1 || { echo "nvm install --lts failed"; exit 1; }
    nvm use --lts > /dev/null 2>&1 || { echo "nvm use --lts failed"; exit 1; }
    set -u
...
install_claude_code() {
    ...
    npm install --global @anthropic-ai/claude-code > /dev/null 2>&1
...
install_gemini() {
    ...
    npm install --global @google/gemini-cli > /dev/null 2>&1
...
install_gtr() {
    ...
    git clone --quiet https://github.com/coderabbitai/git-worktree-runner.git "$tmp_dir/git-worktree-runner" > /dev/null 2>&1
    (cd "$tmp_dir/git-worktree-runner" && sudo ./install.sh > /dev/null 2>&1)
