...
2. **Install dependencies**:
   ```bash
   ./setup.sh                 # installs all tools, prompts for optional ones
   ./setup.sh --skip-optional # installs required tools only
   ```

   **Required** (installed automatically if missing):

   | Tool | Purpose |
   |------|---------|
   | Node.js (via nvm) | Runtime for Claude Code, Gemini CLI |
   ...
   **Optional** (prompted during setup):
   ...
   | Tool | Purpose |
   |------|---------|
   | Salesforce CLI (`sf`) | Customer scheduling workflows |
